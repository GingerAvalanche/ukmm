...
use super::mainfield::{
    ScaleTranslate, collab_anchor::CollabAnchor, korok_location::KorokLocation,
    location_marker::LocationMarker, location_pointer::LocationPointer,
    non_auto_gen_area::NonAutoGenArea, non_auto_placement::NonAutoPlacement,
    restart_pos::RestartPos, road_npc_rest_station::RoadNpcRestStation, start_pos::StartPos,
    static_grudge_location::StaticGrudgeLocation, target_pos_marker::TargetPosMarker,
};
    ...
    fn try_from(byml: &Byml) -> Result<Self> {
        ...
        Ok(Self {
                ...
                .map_or(None, |b| {
                    Some(
                        b.as_array()
                            ...
                            .enumerate()
                            .map(|(index, entry)| {
                                let entry: RestartPos = entry.try_into().with_context(|| {
                                    format!("Could not read RestartPos {}", index)
                                })?;
                                Ok((entry.id(), entry))
                            })
            ...
            location_marker: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: LocationMarker = entry
                        .try_into()
            ...
            location_pointer: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: LocationPointer = entry
                        .try_into()
            ...
            non_auto_gen_area: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: NonAutoGenArea = entry
                        .try_into()
            ...
            non_auto_placement: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: NonAutoPlacement = entry
                        .try_into()
            ...
            road_npc_rest_station: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: RoadNpcRestStation = entry
                        .try_into()
            ...
            start_pos: root_map
                .get("StartPos")
                .ok_or(UKError::MissingBymlKey("MainField static missing StartPos"))?
                .as_array()?
                .iter()
                .enumerate()
                .map(|(index, entry)| {
                    let entry: StartPos = entry
                        .try_into()
            ...
            static_grudge_location: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: StaticGrudgeLocation = entry.try_into().with_context(|| {
                        format!("Could not read StaticGrudgeLocation {}", index)
                    })?;
                    Ok((entry.id(), entry))
                })
                .collect::<Result<DeleteMap<_, _>>>()?,
            target_pos_marker: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: TargetPosMarker = entry
                        .try_into()
            ...
            tera_water_disable: root_map
                ...
                .enumerate()
                .map(|(index, entry)| {
                    let entry: ScaleTranslate = entry
                        .try_into()
...
impl Mergeable for MainStatic {
    fn diff(&self, other: &Self) -> Self {
        ...
        Self {
            ...
            road_npc_rest_station: self
                .road_npc_rest_station
                .deep_diff(&other.road_npc_rest_station),
            start_pos: self.start_pos.deep_diff(&other.start_pos),
            static_grudge_location: self
                .static_grudge_location
                .deep_diff(&other.static_grudge_location),
            ...
            terrain_hide_center_tag: self
                .terrain_hide_center_tag
                .deep_diff(&other.terrain_hide_center_tag),
        }
    }
    ...
    fn merge(&self, diff: &Self) -> Self {
        ...
        Self {
            ...
            road_npc_rest_station: self
                .road_npc_rest_station
                .deep_merge(&diff.road_npc_rest_station),
            start_pos: self.start_pos.deep_merge(&diff.start_pos),
            static_grudge_location: self
                .static_grudge_location
                .deep_merge(&diff.static_grudge_location),
            ...
            terrain_hide_center_tag: self
                .terrain_hide_center_tag
                .deep_merge(&diff.terrain_hide_center_tag),
        }
    }
}
...
mod tests {
    ...
    fn serde() {
        ...
        assert_eq!(
            mstatic.road_npc_rest_station,
            mstatic2.road_npc_rest_station
        );
        assert_eq!(mstatic.start_pos, mstatic2.start_pos);
        assert_eq!(
            mstatic.static_grudge_location,
            mstatic2.static_grudge_location
        );
        ...
        assert_eq!(
            mstatic.terrain_hide_center_tag,
            mstatic2.terrain_hide_center_tag
        );
    }
