...
fn warn_vecf_not_float<T>(val: T)
where
    T: std::fmt::Debug,
{
    log::warn!(
...
pub(crate) fn try_get_vecf(value: &Byml) -> crate::Result<DeleteMap<char, f32>> {
    value
        ...
        .map(|(i, (k, val))| {
            ...
            let key = maybe_key.ok_or(UKError::InvalidByml(
                "Empty or invalid key".into(),
                value.clone(),
            ))?;
            match (key, val) {
                ...
                ('W'..='Z', _) => Err(UKError::InvalidByml(
                    format!("Invalid value for key {key}").into(),
                    value.clone(),
                )),
                (_, Byml::Float(v)) => Err(UKError::InvalidByml(
                    format!("Invalid key for value {v}").into(),
                    value.clone(),
                )),
                (_, Byml::I32(v)) => {
                    warn_vecf_not_float(v);
                    Err(UKError::InvalidByml(
                        format!("Invalid key for value {v}").into(),
                        value.clone(),
                    ))
                }
                ...
                _ => Err(UKError::InvalidByml(
                    format!("Invalid index {i}").into(),
                    value.clone(),
                )),
            }
        })
